...
public class DBManager {
   ...
   public boolean logout(String userna) {
    ...
    public int idOfAuction(String title, int price, String vendor) {
        s = sessionFactory.openSession();

        String sql = "SELECT auctionid FROM Lot where baseprice= :price AND title= :title AND vendor= :vendor";
        try {
            s.beginTransaction();
            NativeQuery sqlQuery = s.createSQLQuery(sql);
            sqlQuery.setParameter("price",price);
            sqlQuery.setParameter("title", title);
            sqlQuery.setParameter("vendor", vendor);
